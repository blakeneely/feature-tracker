...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isBoardFile(body)) {
    return Response.json(
      {
        error:
          'Body must be a well-formed board file: { version: 3, tickets, columns, nextNumber } with columns new/active/resolved plus an optional uat (presence of the uat key = the board has a UAT column), every ticket placed in exactly one column, carrying a unique number below nextNumber and a tags array of unique non-empty strings; the board and any ticket may carry an optional conversation { sessionId, cwd, transcriptPath } of non-empty strings and an optional artifacts array of { title, url } with non-empty title and http(s) url (see CLAUDE.md)',
      },
      { status: 400 },
    );
  }
